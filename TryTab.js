...
import { Text,Pressable, View, Image, StyleSheet, TouchableOpacity, ScrollView } from 'react-native';
...
const TryTab = (props) => {
  ...
  function middleLayer() // dont change it, it updates the middle layer
  {
    ...
    {
      items.push(
      <View key={1} style={[styles.container1,styles.container2]}>
    ...
    {
      items.push(
      <View key={2} style={[styles.container1,styles.container2]}>
...
function correctPressFunction(category, name)//function that saves the chosen object name to the appropriate variable to try on user pic
{
  if (category==1)
  {
...
{
  var comp=<Pressable key={fileN} style={({ pressed }) => [{ height: pressed ? 100 : 70, width: pressed ? 100 : 90, paddingLeft:10,alignSelf:'flex-end', paddingRight:10}]} 
  onPress={()=>{correctPressFunction(fileN.includes("shirt")?1:(fileN.includes("pant")?2:3),fileN)}}>
    <Image style={styles.pic} source={ fileN!=""?{uri:"file://"+RNFS.ExternalDirectoryPath+'/'+fileN}:sample} />
  </Pressable>;
  return comp;
}
